...
class Blog extends CI_Controller {
	...
	public function search(){
		...
		$this->db->select('*');
		$this->db->from('entry');
		$this->db->like('entry.entry_name', $match);
		$this->db->or_like('entry.entry_body', $match);
		
		$this->db->or_like('comment.comment_name',$match);
		$this->db->or_like('comment.comment_email',$match);
		$this->db->or_like('comment.comment_body',$match);
		...
		foreach($results as $result){
			if(!isset($ids[$result->entry_id])){
				$ids[$result->entry_id] = $result->entry_id;
				$entries[] = $result;
			}
		}
